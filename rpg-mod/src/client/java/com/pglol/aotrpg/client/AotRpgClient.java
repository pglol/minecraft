...
public final class AotRpgClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(Net.EstateView.ID, (payload, ctx) -> {
            ClientState.estate = payload;
            if (ctx.client().currentScreen instanceof EstateScreen s) s.refresh();
            else if (payload.open()) ctx.client().setScreen(new EstateScreen());
        });
        ...
        ClientPlayNetworking.registerGlobalReceiver(Net.RegimentView.ID, (payload, ctx) -> {
            ...
            else if (payload.open()) ctx.client().setScreen(new RegimentScreen());
        });
        ...
        ClientPlayNetworking.registerGlobalReceiver(Net.Explored.ID, (payload, ctx) -> {
            ...
            ClientState.exploreCell = payload.cell();
            for (long k : payload.cells()) ClientState.explored.add(k);
        });
        ClientPlayNetworking.registerGlobalReceiver(Net.FerryView.ID, (payload, ctx) -> {
            FerryScreen.view = payload;
            if (ctx.client().currentScreen instanceof FerryScreen s) s.refresh();
            else if (payload.open()) ctx.client().setScreen(new FerryScreen());
        });
