...
Story (Act I): every character lives its own story, written as data in `rpg-mod/src/main/resources/story/`. Your origin decides your opening: Shiganshina (live through the fall of Wall Maria, 845), Trost (the refugees arrive; a stealthy loaf for a starving girl), Ragako (the grain tax and the Reclamation), Stohess/Mitras (the interior's corruption and the Church of the Walls), or the Underground (steal ODM gear from the Military Police unseen). Everyone meets at the 104th Cadet Corps and then the Battle of Trost, where some fates can change (Thomas, Marco), before choosing a branch. Choices build relationships, faction standing and who you become (mercy, Paradis vs humanity, duty vs independence); the journal's **Your Story** tab shows your objective, the people you know and their fates, and a chronicle of your deeds.
- Story characters and scene titans are phased: only you (and party members who join your scene) see them. Party members nearby get a **[Join scene]** prompt; the host makes the choices and guests can suggest.
- `/story` shows your objective, `/story begin` takes an existing character to where their story starts (once), `/story join <player>` / `/story leave`. Operators: `/story skip`, `/story reset`, `/story goto <mission>`, `/story reload`.
- Placeholder character skins are generated by `rpg-mod/tools/make_skins.py`; drop a real 64x64 skin with the same name into `assets/aot_rpg/textures/entity/actor/` to replace one.
